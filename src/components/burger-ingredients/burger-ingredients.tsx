...
export const BurgerIngredients = () => {
  ...
  const sauce = ingredients.filter((item) => item.type === IngredientType.SAUCE);
  ...
  const getCoords = (ref: React.RefObject<HTMLElement>) => ref.current!.getBoundingClientRect();
  ...
  return (
    <section className={styles.ingredients}>
      <div className={cn(styles.menu, 'mb-10')}>
        <Tab value={IngredientType.BUN} active={current === IngredientType.BUN} onClick={handleClickTab}>
          Булки
        </Tab>
        <Tab value={IngredientType.SAUCE} active={current === IngredientType.SAUCE} onClick={handleClickTab}>
          Соусы
        </Tab>
        <Tab value={IngredientType.MAIN} active={current === IngredientType.MAIN} onClick={handleClickTab}>
          Начинки
        </Tab>
      </div>
      <section ref={containerRef} className={cn(styles.container, 'custom-scroll')}>
        <Category title='Булки' id={IngredientType.BUN} ingredients={buns} headerRef={bunRef} />
        <Category title='Соусы' id={IngredientType.SAUCE} ingredients={sauce} headerRef={sauceRef} />
        <Category title='Начинки' id={IngredientType.MAIN} ingredients={main} headerRef={mainRef} />
      </section>
    </section>
  );
};
